...
public class Clubgoer extends Thread {
	...
	public AtomicBoolean pause = ClubSimulation.getPause();
	public AtomicBoolean start = ClubSimulation.getStart();
	...
	private void checkPause() {
		synchronized (pause) {
			while (pause.get()) {
				try {pause.wait();} // if simulation is paused, all threads must wait until it is unpaused
	...
	private void startSim() {
		while (!start.get()) {
			try {latch.await();} // create a latch that stops all threads from running until user presses start, which opens the latch and begins the simulation
			catch (InterruptedException e) {}
			if (start.get()) {
				latch.countDown();
				break;
