...
public class ClubGrid {
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
		...
		synchronized (entrance) {
			while (counter.getInside() >= counter.getMax() || entrance.occupied()) {
				try {entrance.wait();} // if the max amount of people are in the club or entrance block is occupied, then all threads must wait outside until there is space
	...
	public void leaveClub(GridBlock currentBlock,PeopleLocation myLocation)   {
			synchronized (entrance){
				...
				entrance.notifyAll(); // notify all threads waiting outside the club that they can enter
			}
		}
